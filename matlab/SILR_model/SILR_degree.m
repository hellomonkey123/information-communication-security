...
for j = 1:length(k)
    for i = 1:N1
        ...
        x_R(j,i+1) = x_R(j,i)+(-tau*x_R(j,i)- (1 - tau)*P_I*x_R(j,i)*x_S(j,i)*k(j) + (1 -beta)*P_R*x_L(j,i)*x_S(j,i)*k(j) ......
        + (1 - alpha)*P_R*(1 - x_S(j,i))*k(j)*x_S(j,i) + (1 - delta)*P_R*x_I(j,i)*k(j)*x_S(j,i))*t;
        x_I(j,i+1) = x_I(j,i)+(-delta*x_I(j,i)- (1 - delta)*P_R*x_I(j,i)*x_S(j,i)*k(j) + (1 -tau)*P_I*x_R(j,i)*x_S(j,i)*k(j) ......
